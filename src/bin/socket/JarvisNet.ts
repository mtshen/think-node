/**
 * jarvisNet信息只包含 id, option, net
 * 当用户请求时, 只会返回id, name, option
 */
import jarvisId from './jarvisId';
import * as crypto from 'crypto'; 

// 数据传输规则/协议
const jarvisReg: RegExp = /^jarvis\*(\w+)\*([\w\\\/\.]+)\*([0-9]{0,3}):(.{0,})$/;
...
class JarvisNet {
    ...
    private netWaitList: Array<any>;
    private receiveList: Array<any>;
    private outCallback: Function;
    private tryCallback: Function;
     ...
     */
    constructor(netNode, callback: Function) {
        // 属性
    ...
    init(callback: Function) {
        ...
        // 当net获取到数据的回调函数
        netNode.on('data', function(news) {

            let {id, url, state, data}: Agreement = parseAgreement(news.toString());
            // 如果失败, 抛出异常
            if (!id) {
                    ...
                    if (receiveData.url === url) {
                        ...
                        if (state === '001') {
                            receiveFn = function(receiveData = {}) {
                                const bufferContent = new Buffer(stringify(id, 'J', receiveData, '000'), 'utf8');
                                netNode.write(bufferContent);
                                receiveFn = null;                            
                            }
                        }
    ...
    // 获取jarvis格式的数据
    stringify(id: string, url: string, json: object | string, state: string): string {
     ...
     * @param {*} data 需要解析的字符串
     */
    parseAgreement(jarvisData): Agreement {
    ...
    // 发送数据, 只发送数据, 不负责响应
    // data, callback 为选填
    send(url: string, data?: Function | object | string, callback?: Function) {
        const id = jarvisId();
        ...
        if ($callback) {
            this.netNode.write(this.stringify(id, url, $data, '001'));
            this.netWaitList.push({id, $callback});
        } else {
            this.netNode.write(this.stringify(id, url, $data, '002'));
        }
    }
...
function JSONparse(data: string): Object {
    let json = null;
    try {
